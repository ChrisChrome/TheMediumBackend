...
local stageLightStatus = 0
local hallLightStatus  = 0
output.set("yellow", false)
output.set("lightBlue", false)
term.setTextColor(colors.purple)
print("Quantum Server Starting Up")
term.setTextColor(colors.yellow)
temp = fs.open("logo.txt", "r")
print(temp.readAll())
temp.close()
term.setTextColor(colors.purple)
...
print("GitHub: https://github.com/ChrisChrome/TheMediumBackend/")
...
while true do
    ...
    elseif action == "panic" then
        ...
        d4.setURL("about:blank")
        d5.setURL("about:blank")
        commands.title('@a subtitle {"text":""}')
        commands.title('@a title {"text":""}')
        commands.tp("@a 698 21 -451")
        os.shutdown()
    elseif action == "toggleLight" then
        if data == "stage" then
            if stageLightStatus == 0 then
                stageLightStatus = 1
                output.set("lightBlue", true)
            elseif stageLightStatus == 1 then
                stageLightStatus = 0
                output.set("lightBlue", false)
            end
        elseif data == "hall" then
            if hallLightStatus == 0 then
                hallLightStatus = 1
                output.set("yellow", true)
            elseif hallLightStatus == 1 then
                hallLightStatus = 0
                output.set("yellow", false)
            end
        end
